...
export default function WaitlistForm({ lang, session, compact }) {
  ...
  const submit = async () => {
    ...
    const { error: insErr } = await supabase
      .from("waitlist")
      .insert({ email: mail, country: country || null, consent_at: new Date().toISOString() });
      ...
      <div style={{ background:P.panel, border:`1px solid ${P.border}`, borderRadius:6, padding:24,
        ...
        <label style={{ display:"flex", gap:10, alignItems:"flex-start", cursor:"pointer" }}>
          <input
            ...
            style={{ width:16, height:16, marginTop:2, accentColor:P.accent, flexShrink:0, cursor:"pointer" }}
          />
          <span style={{ fontSize:12, color:P.dim, lineHeight:1.6 }}>
            {de ? (
              <>
                Ich möchte per E-Mail über den Start informiert werden. Die{" "}
                <a href="/datenschutz" style={{ color:P.accent }}>Datenschutzerklärung</a>{" "}
                habe ich gelesen. Ich kann die Einwilligung jederzeit widerrufen.
              </>
            ) : (
              <>
                I want to be notified by email at launch. I have read the{" "}
                <a href="/privacy" style={{ color:P.accent }}>privacy policy</a>.
        ...
        <button onClick={submit} disabled={!valid || !consent || loading} style={{
          background:P.accent, border:"none", color:"#1B1E15",
          padding:"14px 0", borderRadius:4, fontSize:14, fontWeight:700,
          fontFamily:"Inter,sans-serif", letterSpacing:"0.04em",
          cursor: valid && consent && !loading ? "pointer" : "default",
          opacity: valid && consent && !loading ? 1 : 0.4,
        }}>
        ...
        <p style={{ fontSize:11, color:P.dim, lineHeight:1.6, margin:0 }}>
          {de
            ? "Deine Adresse wird ausschließlich für die Benachrichtigung zum Start verwendet. Kein Spam, keine Weitergabe."
            : "Your address is used solely to notify you at launch. No spam, no sharing."}
        </p>
      </div>
    </div>
  );
